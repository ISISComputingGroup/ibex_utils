...
def doAxis(config: configparser.ConfigParser, galil: str, axis: str, skips: dict) -> None:
    ...
    doValue(
        f"{mn}.ICOF", config.getfloat(axisItem(axis, "ki")) / 2047.875
    ...
    doValue(
        f"{mn}_MTRTYPE_CMD",
        MOTOR_TYPES[config.getint(axisItem(axis, "motor type"))],
        f"{mn}_MTRTYPE_STATUS",
    )
    doValue(
        f"{mn}_MENCTYPE_CMD",
        ENCODER_TYPES[config.getint(axisItem(axis, "encoder type"))],
    ...
    print(
        f"INFO: {mn} seci home method: ", HOME_METHODS[config.getint(axisItem(axis, "home method"))]
    )
...
# Stall Allowed Error
def axisItem(axis: str, name: str) -> str:
    return f"axis {axis} {name}"


def doValue(set_pv: str, value: str, read_pv: str | type.NoneType = None) -> None:
...
def doController(config, galil: str, skips: dict) -> None:
...
# G0 -> 01 etc.
def controllerNumber(galil: str) -> str:
    if galil[0] == "G":
        return "{:02d}".format(int(galil[1]) + 1)


# construct MTRxxyy, axis 'a' -> 01
def motorNumber(galil: str, axis: str) -> str:
...
def compareValues(val1: float | int, val2: float | int) -> bool:
    tolerance = 0.003
    try:
        v1 = float(val1)
        v2 = float(val2)
