...
class TestIocCopier(unittest.TestCase):
    ...
    def test_check_valid_ioc_to_copy_AND_st_common_AND_no_seq_THEN_not_exit(
        self, mock_exists, mock_print
    ):
    ...
    @mock.patch("builtins.print")
    @mock.patch.object(
        ioc_copier.sys, "argv", ["ioc_copier", "test", "test", "test", "test", "test"]
    )
    ...
    def test_handle_arguments_called_AND_more_than_4_arguments_THEN_return_variables(
        self, mock_print
    ):
    ...
    @parameterized.expand(
        [
            ...
            (
                "handle_length",
                ["test_02", "IOC_02", "IOC-02", "test_2", "IOC_2", "IOC-2"],
                ["test_13", "IOC_13", "IOC-13", "test_13", "IOC_13", "IOC-13"],
                13,
                0,
            ),
    ...
    @mock.patch("builtins.print")
    def test_replace_text_called_THEN_replace_text_start_in_text_with_current(
        self, _, test_text, check_text, copy, print_called, mock_print
    ):
    ...
    @parameterized.expand(
        [
            (
                ...
                1,
                "st.cmd",
                "test_01\n< iocBoot/ioctest-IOC-01/st-common.cmd",
                ["test_03\n", "< iocBoot/ioctest-IOC-01/st-common.cmd"],
                0,
            ),
            (
                ...
                2,
                "st.cmd",
                "test_02\n< iocBoot/ioctest-IOC-02/st-common.cmd",
                ["test_03\n", "< iocBoot/ioctest-IOC-03/st-common.cmd"],
                0,
            ),
            (
                "_with_src_make",
    ...
    def test_get_file_text_called_THEN_handle_all_casses(
        self, _, start, file, read, result, comment, mock_comment
    ):
    ...
    def test_copy_folder_AND_valid_ioc_name_and_format_THEN_correct_copy(self, mock_copy):
        ...
        path = os.path.join(
            os.getcwd(), file_format.format(f"{ioc_name}-{ioc_copier.padded_current_copy}")
        )
    ...
    def test_copy_loop_THEN_writes_to_file(
        self,
        _,
        start_copy,
        initial_copy,
        ...
        with mock.patch("builtins.open", mock_open):
            mock_walk.return_value = [
                (
                    "/iocBoot",
                    (f"subfolder_{padded_start_copy}",),
                    (f"test_{padded_start_copy}.db",),
                ),
                (
            ...
            mock_open.assert_called_with(
                f"/iocBoot/subfolder_{padded_start_copy}\\test_{padded_start_copy}.proto", "w"
            )
            file_handle = mock_open.return_value.__enter__.return_value
            file_handle.writelines.assert_called_with(
                [f"test_{padded_max_copy}\n", f"IOC-{max_copy}"]
            )
            mock_rename.assert_called_with(
                f"/iocBoot/subfolder_{padded_start_copy}", f"test_{padded_start_copy}.proto", "test"
            )
    ...
    def test_copy_loop_AND_valid_parameters_AND_empty_THEN_copy_folder_AND_end(
        self, mock_open, mock_walk, mock_rename, mock_folder
    ):
        ioc_copier.START_COPY = 2
        initial_copy = 3
        max_copy = 5
    ...
    @mock.patch("ioc_copier.os.walk")
    @mock.patch("builtins.open", new_callable=mock.mock_open, read_data="test_02\nIOC-2")
    def test_copy_loop_AND_max_is_start_AND_empty_THEN_copy_folder_AND_end(
        self, mock_open, mock_walk, mock_rename, mock_folder
    ):
        ioc_copier.START_COPY = 2
        initial_copy = 3
