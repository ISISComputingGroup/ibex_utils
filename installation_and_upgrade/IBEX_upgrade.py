...
def _get_latest_release_path(release_dir: str) -> str:
    ...
    releases = [
        name for name in os.listdir(release_dir) if os.path.isdir(os.path.join(release_dir, name))
    ]
...
def _get_latest_existing_dir_path(release_dir: str, component: str) -> str:
    ...
    releases = sorted(
        list(filter(regex.match, releases)), key=semantic_version.Version, reverse=True
    )
...
DIRECTORIES = {"EPICS": "", "Client": "", "genie_python_3": ""}

if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Upgrade the instrument.", formatter_class=argparse.RawTextHelpFormatter
    )
    ...
    parser.add_argument(
        "--server_dir", default=None, help="Directory from which IBEX server should be installed"
    )
    parser.add_argument(
        "--client_dir", default=None, help="Directory from which IBEX client should be installed"
    )
    parser.add_argument(
        "--genie_python3_dir",
        default=None,
    ...
    if args.release_dir is not None:
        ...
        for key in DIRECTORIES:
            if not os.path.isdir(DIRECTORIES[key]):
                ...
                if key == "EPICS":
                    ...
                    DIRECTORIES[key] = _get_latest_existing_dir_path(
                        args.release_dir, key + server_suffix
                    )
                else:
        ...
        if partial_release:
            try:
                ...
                missing_prompt.prompt_and_raise_if_not_yes(
                    f"Would you like to use Server version: {server_version}, "
        ...
        try:
            DIRECTORIES["EPICS"] = FileUtils.get_latest_directory_path(
                epics_build_dir, "BUILD-", "EPICS" + server_suffix
            )
            ...
            DIRECTORIES["genie_python_3"] = FileUtils.get_latest_directory_path(
                genie_python3_build_dir, "BUILD-"
            )
        except IOError as e:
            print(e)
            sys.exit(3)
        ...
        args.server_dir is not None
        and args.client_dir is not None
        and args.genie_python3_dir is not None
    ):
        ...
        print(
            "You must specify either the release directory or kits_icp_dir or "
