...
def main(settings, token):
    ...
    try:
        ...
        if settings.date_from is not None or settings.ms_close:
            action = True
            isis_repo_access.update_milestones(
                settings.date_from, settings.date_to, settings.ms_close
            )
        ...
        with open(filename) as file:
            for line in file:
                ...
                if label_line != "" and not is_comment:
                    ...
                    if len(split_line) != 2:
                        raise UserError(
                            f"Label lines must be <colour code>, <label name>. Line: {label_line}"
                        )
...
def _parse_command_line():
    ...
    parser.add_argument(
        "--dry-run",
        action="store_true",
        help="Don't change anything just tell me what you would do.",
    )
    ...
    parser.add_argument(
        ...
        "--owner",
        required=True,
        default="ISISComputingGroup",
        help="Owner of the repositories",
    )
    parser.add_argument(
        "--repo-file",
        required=False,
    ...
    # milestone creation
    parser.add_argument(
        "--ms-from", required=False, dest="date_from", help="Date from which the sprint starts."
    )
    parser.add_argument(
        "--ms-to", required=False, dest="date_to", help="Date to which the sprint runs."
    )
    ...
    if len(sys.argv) == 1:
        sys.stderr.write(
            "You need to use commandline arguments and you passed none. If in windows make sure your *.py "
...
if __name__ == "__main__":
    exit(main(_parse_command_line(), _get_user_token()))
